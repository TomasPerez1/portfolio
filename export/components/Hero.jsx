...
export default function Hero({ data, showStatus = true }) {
  const [tilt, setTilt] = useState({ x: -22, y: 28 });
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (autoOn) { t += 0.005; setTilt({ x: -22 + Math.sin(t) * 4, y: 28 + Math.cos(t * 0.7) * 8 }); }
...
function VoxelArt({ tilt }) {
  const SIZE = 4, V = 36;
  const yellow = new Set(["1,3,1","2,3,1","3,3,1","2,2,1","2,1,1","2,0,1","1,3,2","2,3,2","3,3,2","2,2,2","2,1,2","2,0,2"]);
  const removed = new Set(["0,0,0","0,1,0","0,2,3","3,0,3","3,3,0","0,3,3","3,2,0","1,0,3","2,3,3","0,2,0","3,1,3"]);
  const voxels = [];
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      ...
      >
        {voxels.map((v) => <Voxel key={v.key} {...v} size={V} />)}
      </div>
    </div>
  );
}

function Voxel({ x, y, z, size, isYellow }) {
  const base = isYellow ? "#fbbf24" : "var(--c-spark)";
  const tx = (x - 1.5) * size, ty = -((y - 1.5) * size), tz = (z - 1.5) * size;
  const faces = [
